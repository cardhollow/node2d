(() => {
  ...
  function runtimeApproachVelocity(body,desired,dt,rate){
    const current={x:Number(body.vx)||0,y:Number(body.vy)||0};
    const maxDelta=Math.max(0,Number(rate)||0)*Math.max(0,Number(dt)||0);
    const delta=subV(desired,current),d=Math.hypot(delta.x,delta.y);
    if(d<=maxDelta||maxDelta<=0){body.vx=desired.x;body.vy=desired.y;return;}
    const k=maxDelta/d;body.vx=current.x+delta.x*k;body.vy=current.y+delta.y*k;
  }
  function runtimeSmoothFace(body,desired,dt,maxTurn=720){
    if(body?.physics?.fixedRotation||!desired||Math.hypot(desired.x,desired.y)<1e-6)return;
    const targetAngle=Math.atan2(desired.y,desired.x)*180/Math.PI;
    let delta=((targetAngle-(Number(body.t?.angle?.[0])||0)+540)%360)-180;
    const step=Math.max(0,Number(maxTurn)||0)*Math.max(0,Number(dt)||0);
    delta=clamp(delta,-step,step);body.t.angle[0]=(Number(body.t?.angle?.[0])||0)+delta;
  }
  function runtimeSteeringForAI(body,ai,dt,bodyById){
    ...
    const lookAhead=Math.max(20,Math.min(72,speed*.22+bodyRadius*2));
    let travel=0,last=pos;
    for(let i=ai.pathIndex;i<ai.path.length;i++){
      const point=ai.path[i],seg=Math.hypot(point.x-last.x,point.y-last.y);travel+=seg;last=point;
      if(travel>=lookAhead){waypoint=point;break;}
      waypoint=point;
    }
